// api/mcp.js
// Explicit MCP tools for selected Amello endpoints.
// Deploy on Vercel as a Serverless Function (Node 18+).
// Install: npm i @modelcontextprotocol/sdk zod

const { McpServer } = require('@modelcontextprotocol/sdk/server/mcp.cjs');
const { StreamableHTTPServerTransport } = require('@modelcontextprotocol/sdk/server/streamableHttp.cjs');
const { z } = require('zod');
...
//    Get hotel offers for multiple rooms (or framework if roomConfigurations = []).
const HotelOfferBodySchema = z.object({
  ...
  roomConfigurations: MultiroomConfigsSchema, // [] allowed => framework data only
  locale: LocaleEnum
});

// 2) POST /api/v1/find-hotels
//    Find hotels by transmitted POST data (region search)
...
server.registerTool(
  'amello.currencies_get',
  {
    ...
    inputSchema: z.object({
      query: CurrenciesGetQuerySchema,
      headers: z.record(z.string()).optional()
    }),
    outputSchema: z.object({
      status: z.number(),
